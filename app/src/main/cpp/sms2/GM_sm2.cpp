...
//	ZA_SRC_Buff = new unsigned char [lenZA_SRC + MAX_STRLEN] ;
	ZA_SRC_Buff = (unsigned char *) malloc((lenZA_SRC + MAX_STRLEN) * sizeof(unsigned char));
...
//	pM_A = new unsigned char[32+lenSrc+MAX_STRLEN];
	pM_A = (unsigned char *) malloc((32 + lenSrc + MAX_STRLEN) * sizeof(unsigned char));
	if (NULL == pM_A)
	{
		ret =  ERR_MEM_ALLOC;
		goto END;
...
//	src_strbuff = new char [lenSrc*2 + MAX_STRLEN];
	src_strbuff = (char *) malloc((lenSrc * 2 + MAX_STRLEN) * sizeof(char));
	if (NULL == src_strbuff)
	{
...
//	unsigned char * pZandCt = new unsigned char [ulZlen + 4 + 10];
	unsigned char *pZandCt = (unsigned char *) malloc((ulZlen + 4 + 10) * sizeof(unsigned char));
	if (NULL == pZandCt)
	{
		return ERR_MEM_ALLOC;
	}
...
//	pC2 = new unsigned char[C2_len+10];
	pC2 = (unsigned char *) malloc((C2_len + 10) * sizeof(unsigned char));
	if (NULL == pC2)
	{
		return ERR_MEM_ALLOC;
	}
...
//	pout = new unsigned char [C2_len + 10];
    pout = (unsigned char *) malloc((C2_len + 10) * sizeof(unsigned char));
	if(NULL == pout)
	{
		ret = ERR_MEM_ALLOC;
		goto END;
...
//	ptmp = new unsigned char[C2_len + tmpX2Len + tmpY2Len + 100];
	ptmp =  (unsigned char*)malloc((C2_len + tmpX2Len + tmpY2Len + 100)* sizeof(unsigned char));
	if (NULL == ptmp)
	{
		ret = ERR_MEM_ALLOC;
		goto END;
...
//		pout = new unsigned char [plainLen + 10];
		pout = (unsigned char*)malloc((plainLen + 10)* sizeof(unsigned char));
		if(NULL == pout)
		{
			ret = ERR_MEM_ALLOC;
			goto END;
...
//	C2_buf = new unsigned char [plainLen + 10];
	C2_buf = (unsigned char*)malloc((plainLen + 10)* sizeof(unsigned char));
	if (NULL == C2_buf)
	{
		//delete [] C2_buf;
		ret = ERR_MEM_ALLOC;
...
//	ptmp = new unsigned char[plainLen + tmpX2Len + tmpY2Len + 100];
	ptmp = (unsigned char*)malloc((plainLen + tmpX2Len + tmpY2Len + 100)* sizeof(unsigned char));
	if (NULL == ptmp)
	{
		ret = ERR_MEM_ALLOC;
		goto END;
